...
chrome.spawn = function(options) {
	return new Promise((resolve, reject) => {
		...
		if (!fs.existsSync(location)) {
			util.error('unable to find Chrome install. Please specify with chromeLocation');
			return reject();
		}
	...
	return new Promise((resolve, reject) => {
		...
		let connect = () => {
			CDP.Version({ port: this.options.browserDebuggingPort }).then((info) => {
				...
				util.warn('retrying connection to Chrome...');
		...
		Promise.all([
			...
			Security.certificateError(({eventId}) => {
				Security.handleCertificateError({
					...
					util.error('error handling certificate error:', err);
				});
			});
...
chrome.parseHtmlFromPage = function(tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			util.error('unable to parse HTML', err);
			tab.prerender.statusCode = 504;
			clearTimeout(parseTimeout);
			reject();
		});
